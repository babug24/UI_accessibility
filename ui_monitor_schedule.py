...
EADERS = {
    ...
    "Accept": "Htext/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
}
        ...
        with open(csv_file, newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                if first.lower() in ("url", "site"):
                    continue
                url = first
                # allow rows with only URL (use default interval)
                interval = None
...
def check_url(url):
    try:
        ...
        if resp.status_code >= 400:
            return 'DOWN', f'status {resp.status_code}'
        if is_site_down(resp.text):
            return 'DOWN', 'indicator found'
        return 'UP', f'status {resp.status_code}'
...
def monitor(csv_file):
    ...
    # next_check stores the epoch time when each url should next be checked
    next_check = {url: time.time() for url in url_intervals}
    # last_status to optionally only announce on change (can be useful)
        ...
        while True:
            ...
            for url, interval in url_intervals.items():
                if now >= next_check[url]:
                    ...
                    # print full status each time; change-only behavior can be enabled by checking last_status
